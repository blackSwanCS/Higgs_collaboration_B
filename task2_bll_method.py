...
def bll_method(labels, scores, weights, N_bins=25):
    ...
    def gamma(alpha_tes, alpha_jes):
        ...
        return [
            (gamma_alpha_jes[k] + gamma_alpha_tes[k])
            for k in range(len(gamma_alpha_tes))
        ]
    ...
    def beta(
        ...
        return [
            (beta_alpha_jes[k] + beta_alpha_tes[k]) for k in range(len(beta_alpha_tes))
        ]

    def BinContent(k, mu, alpha_tes, alpha_jes):
        return (
            mu * gamma(alpha_tes, alpha_jes)[k] * pS[k]
            + beta(alpha_tes, alpha_jes)[k] * pB[k]
        )
    ...
    def bll(mu, alpha_tes, alpha_jes):
        ...
        alpha0 = 1
        return (
            -2 * sum([np.log(likp(k, y[k], mu, alpha_tes, alpha_jes))])
